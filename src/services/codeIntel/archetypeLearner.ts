...
function buildPrompt(group: PackageGroup, sampleCount: number): string {
  return [
    ...
    `  controller, service, repository, mapper, dto, filter, middleware,`,
    `  config, test, main, util, unknown.`,
    ``,
    `If a file is something other than the above, pick "unknown" and mention it`,
    `briefly in <confidence> as a short tag (e.g. "low|schema", "low|dataclass").`,
    ``,
...
function normaliseArchetype(raw: string): Archetype {
  ...
  const known: Archetype[] = [
    ...
    'filter', 'middleware', 'config', 'test', 'main', 'util', 'unknown',
  ];
